    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              && !(pos.must_capture() && pos.attackers_to(to_sq(move), ~us)))
          {
              ...
              if (!pos.see_ge(move, Value(-(30 - std::min(lmrDepth, 18) + 10 * !!pos.capture_the_flag_piece() + 10 * pos.blast_on_capture() + 10 * pos.check_counting() + 10 * (pos.count<KING>() == 1)) * lmrDepth * lmrDepth)))
                  continue;
          }
